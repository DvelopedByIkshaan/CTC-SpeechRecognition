...
"""Library for performing CTC decoding, online and offline."""

import sys

if sys.version_info < (3, 5):
    if sys.version_info > (3, 7):
        print("THIS MODULE REQUIRES PYTHON 3.5, 3.6, OR 3.7 YOU ARE CURRENTLY USING PYTHON {0}".format(sys.version))
        print("as soon as possible this module will be able to support python 3.8 and python 3.9+.")
    else:
        print("THIS MODULE REQUIRES PYTHON 3.5, 3.6, OR 3.7 YOU ARE CURRENTLY USING PYTHON {0}".format(sys.version))
    sys.exit(1)
if sys.version_info == (3, 7):
    print("beacuse of pyaudio is not supported in python 3.6+ version the unofficial pyaudio modules are being installed so engine would predict the results acuratly so make sure you have python 3.5 or 3.6 install in your system")

import torch
from .ext import ctc_decode
...
class CTCBeamDecoder(object):
    ...
    def __init__(self, labels, model_path=None, alpha=0, beta=0, cutoff_top_n=40, cutoff_prob=1.0, beam_width=100,
        ...
        if model_path:
            self._scorer = (alpha, beta, model_path.encode(), self._labels,
                            self._num_labels)
        self._cutoff_prob = cutoff_prob

    def decode(self, probs, fname, seq_lens=None):
        """
        ...
        if self._scorer:
            th_output, th_scores, th_timesteps, th_out_seq_len = ctc_decode.paddle_beam_decode_lm(probs, seq_lens, self._labels, self._num_labels, self._beam_width,
                                             self._num_processes, self._cutoff_prob, self.cutoff_top_n, self._blank_id,
                                             self._log_probs, self._scorer, output, timesteps, scores, out_seq_len, fname)
        else:
            th_output, th_scores, th_timesteps, th_out_seq_len = ctc_decode.paddle_beam_decode(probs, seq_lens, self._labels, self._num_labels, self._beam_width,
                                          self._num_processes, self._cutoff_prob, self.cutoff_top_n, self._blank_id,
                                          self._log_probs, output, timesteps, scores, out_seq_len, fname)
            output, scores, timesteps, out_seq_len = th_output, th_scores, th_timesteps, th_out_seq_len
